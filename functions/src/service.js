...
const getUrl = async lot => {
    ...
    const data = await page.goto(
        `https://www.autobidmaster.com/en/carfinder-online-auto-auctions/lot/${lot}`,
    );
    await browser.close();
    return data._url;
};
...
const getBody = async lot => {
    ...
    const divContentFull = childrenBody
        .toArray()
        .filter(element => $(element).attr().class === '');
    const divContent = $($(divContentFull).children()[0]).children()[1];
    const divRealContent = $(
        $($(divContent).children()[1]).children()[0],
    ).children()[1];

    const vinDataHtml = $(
        $($(divRealContent).children()[0]).children()[1],
    ).children()[0];
    const additionalDataHtml = $(
        $($(divRealContent).children()[0]).children()[2],
    ).children()[0];
    const vinData = [];
